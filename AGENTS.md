...
- **No screenshot automation, and the screenshots on disk are the 2010 app's.** `fastlane/screenshots/` holds twenty PNGs carried over with the rewrite — 640×960 (iPhone 4) and iPad 9.7, sizes App Store Connect no longer accepts, showing a UI that no longer exists. Nothing uploads them: `skip_screenshots: true` is deliberate, because that path goes through iTMSTransporter and fails often enough to make the workflow untrustworthy for the part that does work. **3.0.0 needs a fresh set, captured and uploaded by hand.**
...
- **There is no `fastlane/metadata/review_information/`, and adding it back empty breaks the upload.** `deliver init` writes seven blank placeholder files there, and deliver sends the review detail whenever that directory holds files at all — App Store Connect then refuses the request with "You must provide a value for the attribute 'contactFirstName'". The contact is a person's name, phone number and address; it belongs in App Store Connect, not in a public repository. The validator's rule is all four contact fields or none.
