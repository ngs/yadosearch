...
- **UI は日本語と英語。** 日本語を原文とし、日本語の文字列そのものをキーにして `Resources/Localizable.xcstrings` で英語に対応づけています。String Catalog は UI パッケージではなくアプリターゲットに置いています。SwiftUI は `LocalizedStringKey` を既定で `Bundle.main` から引くので、全呼び出し箇所に `bundle: .module` を書かずに済むためです（プレビューと `swift test` ではカタログが無く、キー＝日本語にフォールバックします）。`String` 型の値は自動では引かれないので、`Amenity.title` や `searchErrorMessage(for:)` などは `String(localized:)` で包んでいます。
- **プロキシが返す内容は日本語のままです。** 宿名・エリア名・サービスのエラーメッセージは上流が日本語しか持たないため、端末の言語設定に関わらず日本語で表示されます。逆ジオコーディングの地名だけは端末の言語に追随します。
